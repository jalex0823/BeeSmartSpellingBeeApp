...
(function(){
  function initHoneyLoader() {
    ...
    // IMPORTANT: run real tasks
    const emergencyBypass = false; // was true

    // Reduced motion support
    const prefersReduced = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (prefersReduced) overlay.classList.add('reduced-motion');

    // Matrix Rain Animation Setup - non-blocking and guarded
    ...
    // System checks with realistic timing
    const tasks = [
      {
        name: 'System Health',
        weight: 20,
