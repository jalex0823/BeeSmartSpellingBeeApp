"""
Generate iOS splash screen images with BeeSmart branding
Replaces Flutter logo with BeeSmart logo for App Store compliance
"""
...
def create_ios_splash_screens():
    """Generate iOS splash screens with BeeSmart logo"""
    print("Generating iOS splash screens with BeeSmart branding...")
    
    # Paths
    base_dir = os.path.dirname(os.path.abspath(__file__))
    logo_path = os.path.join(base_dir, 'static', 'BeeSmartCrestLogo1.png')
    splash_dir = os.path.join(base_dir, 'mobile', 'ios', 'App', 'App', 'Assets.xcassets', 'Splash.imageset')
    ...
    # Splash screen size (2732x2732 for all scales)
    splash_size = 2732
    gold_color = (255, 215, 0)  # #FFD700 - BeeSmart gold
    
    # Create splash screen with gold background
    splash = Image.new('RGB', (splash_size, splash_size), gold_color)
    
    # Calculate logo scaling (60% of screen width, maintaining aspect ratio)
    max_logo_size = int(splash_size * 0.6)
    ...
    x = (splash_size - new_width) // 2
    y = (splash_size - new_height) // 2
    ...
    # Save all three versions (they're all the same size, but iOS expects them)
    filenames = [
